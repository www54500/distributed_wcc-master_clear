...
public class StartWccIterationComputation extends AbstractComputation<
  ...
  public void compute(
    ...
    vData.setCommInitNeighbors(new CommunityInitMessageArrayWritable(new CommunityInitializationMessage[0])); // Don't need it anymore 
    vData.saveCurrentCommunityAsBest();

    sendMessageToAllEdges(vertex, new TransferMessage(vertex.getId().get(), vData.getCommunity()));
  }
}
